...
namespace Frends.Community.Azure.Blob
{
    public class Download
    {
        ...
        {
            var result = await DownloadBlob(source, destination, null, SourceBlobOperation.Download, cancellationToken);
        ...
        public static async Task<ReadContentOutput> ReadBlobContentAsync(SourceProperties source, BlobContentProperties contentProperties, CancellationToken cancellationToken)
        {
            var result = await DownloadBlob(source, null, contentProperties, SourceBlobOperation.Read, cancellationToken);
        ...
        private static async Task<DownloadOutputBase> DownloadBlob(SourceProperties sourceProperties, DestinationFileProperties destinationProperties, BlobContentProperties contentProperties, SourceBlobOperation operation, CancellationToken cancellationToken)
        {
            ...
            switch (operation)
            {
                case SourceBlobOperation.Read:
                    return await ReadContents(blobReference, contentProperties.EncodingName);
                // default is to donwload blob
                default:
                    return await WriteToFile(blobReference, sourceProperties.BlobName, destinationProperties, cancellationToken);
            }

        }

        private static async Task<DownloadOutputBase> ReadContents(CloudBlob blobReference, string blobEncoding)
        {
            using(var memoryStream = new MemoryStream())
            {
                await blobReference.DownloadToStreamAsync(memoryStream);
                Encoding encoding = Encoding.GetEncoding(blobEncoding);
                return new DownloadOutputBase { Content = encoding.GetString(memoryStream.ToArray()) };
            }
        }

        private static async Task<DownloadOutputBase> WriteToFile(CloudBlob blobReference, string blobName, DestinationFileProperties destinationProperties, CancellationToken cancellationToken)
        {
            var destinationFileName = blobName;
            ...
            await blobReference.DownloadToFileAsync(destinationFileFullPath, FileMode.Create, cancellationToken);
    ...
    public class BlobContentProperties
    {
        /// <summary>
        /// Encoding in which blob content is read.
        /// </summary>
        [DefaultDisplayType(DisplayType.Text)]
        [DefaultValue("UTF-8")]
